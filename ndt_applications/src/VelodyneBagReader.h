...
To rawdata.h is added int setupOffline(std::string calibration_file);

The implementation of this is in rawdata.cc as follows:

  int RawData::setupOffline(std::string calibration_file)
  {
 
		config_.max_range = (double) velodyne_rawdata::DISTANCE_MAX;
		config_.min_range = 2.0;
...
class VelodyneBagReader{
											...
											double velodyne_min_range=2.0) 
	{
		...
		fprintf(stderr,"Searched Topic = '%s'\n",velodynetopic_.c_str());
	...
	bool readNextMeasurement(pcl::PointCloud<pcl::PointXYZI> &cloud, tf::Transform &sensor_pose){
		if(I == view->end()){
	...
	bool readMultipleMeasurements(unsigned int Nmeas, pcl::PointCloud<pcl::PointXYZI> &cloud, tf::Transform &sensor_pose){
		tf::Transform T;
		ros::Time t0;
		velodyne_rawdata::VPointCloud pnts,conv_points;
	...
	bool readMultipleMeasurements(unsigned int Nmeas, pcl::PointCloud<pcl::PointXYZI> &cloud, tf::Transform &sensor_pose, tf::Transform &base_pose, std::string base_pose_id){
