...
// 			double getCellOrientation(const lslgeneric::NDTCell& cell) const ;
			 ...
			 */
			std::vector< NDTCornerBundle > getAllCorners(const lslgeneric::NDTMap& map, const Eigen::Vector3d& robot_pose) ;
			
			/**
			 * @brief return all the cells around cell that possess a gaussian and are maximum the resolution of the map away.
			 */
// 			void getClosestCells(const lslgeneric::NDTMap& map, const lslgeneric::NDTCell& cell, int neig_size, std::vector< boost::shared_ptr< lslgeneric::NDTCell > >& cells_gaussian, std::vector< boost::shared_ptr< lslgeneric::NDTCell > >& cells_initialized) const ;
...
// 			}
		...
		private:
			...
			bool gotAngledNDT(const lslgeneric::NDTMap& map, const std::vector< boost::shared_ptr< lslgeneric::NDTCell > >& neighbor, const std::vector< boost::shared_ptr< lslgeneric::NDTCell > >& cells_initialized, NDTCornerBundle& corner) ;
			/**
