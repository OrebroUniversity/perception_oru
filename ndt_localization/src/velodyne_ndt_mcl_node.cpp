...
class particle_filter_wrap {
	...
	void VeloCallback(const velodyne_msgs::VelodyneScan::ConstPtr& msg){
	  //ROS_INFO_STREAM("VELO recived");
		...
		try{
		  tf_listener.lookupTransform(odomTF, "/base_link",/*ros::Time(0)*/msg->header.stamp, transform);
...
// lslgeneric::NDTMap PointCloudToNDT (const sensor_msgs::PointCloud2::ConstPtr& scan){
